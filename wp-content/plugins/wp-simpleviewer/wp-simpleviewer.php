...
class SimpleViewer {
	...
	function __construct() {
		...
		add_action('admin_print_scripts', array(&$this, 'add_scripts_admin'));
	 ...
	 */
	function add_menus() {
		add_menu_page('WP-SimpleViewer', 'SimpleViewer', 'edit_plugins', 'sv_manage_gallery', array(&$this, 'edit_galleries_page'), plugins_url('img/favicon.png', __FILE__));
		$edit_galleries_page = add_submenu_page('sv_manage_gallery', 'WP-SimpleViewer - Manage Galleries', 'Manage Galleries', 'edit_plugins', 'sv_manage_gallery', array(&$this, 'edit_galleries_page'));
		$help_page = add_submenu_page('sv_manage_gallery', 'WP-SimpleViewer - Help', 'Help', 'edit_plugins', 'sv_help', array(&$this, 'help_page'));
		add_action('admin_print_scripts-' . $edit_galleries_page, array(&$this, 'add_scripts_custom'));
		add_action('admin_print_styles-' . $edit_galleries_page, array(&$this, 'add_styles_custom'));
	}
...
<?php
	...
	function get_reset_values() {
		...
		$reset_values['languageCode'] = 'AUTO';
	...
	function get_default_values() {
		...
		if (file_exists($default_filename)) {
			$default_xml = simplexml_load_file($default_filename);
			$default_values = $default_xml->attributes();
		}
	...
	*/
	function build_gallery($gallery_filename, $custom_values) {
		...
		if (isset($custom_values['transparent-background']) && $custom_values['transparent-background'] == 'true') {
...
<?php
	...
	function add_scripts_admin() {
		wp_register_script('sv_script_generate', plugins_url('js/generate.js', __FILE__), array('jquery'), $this->version);
				...
				switch ($_GET['sv-action']) {
					...
					case 'gallery-edited':
						...
						if ($gallery_id != '') {
							...
							echo '<div class="updated"><p>Gallery Id cannot be found.</p></div>';
						}
					...
					case 'delete-gallery':
						...
						$this->display_gallery_table();
						break;
					case 'delete-all-galleries':
						$validated = false;
						if ($_GET['referer'] == 'header' && check_admin_referer('sv-delete-header', 'sv-delete-header-nonce')) {
							$validated = true;
						}
						if ($_GET['referer'] == 'footer' && check_admin_referer('sv-delete-footer', 'sv-delete-footer-nonce')) {
							$validated = true;
						}
						if (!$validated) {
							break;
						...
						if (count($galleries) > 0) {
							...
							if (count($galleries) == 0) {
								...
								if ($options['last_id'] != '0') {
									...
									echo '<div class="updated"><p>All galleries successfully deleted and Gallery Id successfully reset to zero.</p></div>';
								} else {
									echo '<div class="updated"><p>Gallery Id already set to zero.</p></div>';
								}
							} else {
								echo '<div class="updated"><p>All galleries cannot be deleted.</p></div>';
							}
						} else {
							echo '<div class="updated"><p>There are no galleries to delete.</p></div>';
						}
						$this->display_gallery_table();
						break;
					case 'set-defaults':
						...
						break;
					case 'defaults-set':
						if (!check_admin_referer('sv-defaults', 'sv-defaults-nonce')) {
							break;
						}
						$default_filename = plugin_dir_path(__FILE__) . 'default.xml';
						$this->build_gallery($default_filename, $_POST);
						echo '<div class="updated"><p>Default values of gallery options successfully set.</p></div>';
						$this->display_gallery_table();
						break;
					case 'reset-defaults':
						$validated = false;
						if ($_GET['referer'] == 'header' && check_admin_referer('sv-reset-header', 'sv-reset-header-nonce')) {
							$validated = true;
						}
						if ($_GET['referer'] == 'footer' && check_admin_referer('sv-reset-footer', 'sv-reset-footer-nonce')) {
							$validated = true;
						}
						if (!$validated) {
							break;
						...
						$default_filename = plugin_dir_path(__FILE__) . 'default.xml';
						$this->build_gallery($default_filename, $this->get_reset_values());
						echo '<div class="updated"><p>Default values of gallery options successfully reset to original values.</p></div>';
						$this->display_gallery_table();
						break;
				}
			} else {
...
?>
		<div id="buttons-header">
			<form class="sv-delete" id="delete-header" action="<?php echo get_bloginfo('wpurl') . '/wp-admin/admin.php'; ?>" method="get" style="display: inline;" title="Delete all galleries and reset Gallery Id to zero.">
				<input class="button" id="delete" name="delete" type="submit" value="Delete All Galleries" />
				<input type="hidden" id="page" name="page" value="sv_manage_gallery" />
				<input type="hidden" id="sv-action" name="sv-action" value="delete-all-galleries" />
				<input type="hidden" id="referer" name="referer" value="header" />
<?php
				wp_nonce_field('sv-delete-header', 'sv-delete-header-nonce');
?>
			</form>
			<input type="button" class="button" id="defaults-header" name="defaults-header" value="Set Defaults" onclick="location.href='<?php echo get_bloginfo('wpurl') . '/wp-admin/admin.php?page=sv_manage_gallery&sv-action=set-defaults'; ?>'" style="display: inline;" title="Set default values for gallery options." />
			<form class="sv-reset" id="reset-header" action="<?php echo get_bloginfo('wpurl') . '/wp-admin/admin.php'; ?>" method="get" style="display: inline;" title="Reset default values of gallery options to original values.">
				...
				<input type="hidden" id="referer" name="referer" value="header" />
<?php
				wp_nonce_field('sv-reset-header', 'sv-reset-header-nonce');
?>
			</form>
		</div>
...
<?php
							...
							$gallery_title = $default_values['title'];
							$dom_doc = new DOMDocument();
							$dom_doc->load($gallery_filename);
							$settings_tags = $dom_doc->getElementsByTagName('simpleviewergallery');
							$settings_tag = $settings_tags->item(0);
							if ($settings_tag->hasAttribute('title')) {
								$gallery_title = $settings_tag->getAttribute('title');
							}
?>
...
?>
		...
		<div id="buttons-footer">
			<form class="sv-delete" id="delete-footer" action="<?php echo get_bloginfo('wpurl') . '/wp-admin/admin.php'; ?>" method="get" style="display: inline;" title="Delete all galleries and reset Gallery Id to zero.">
				<input class="button" id="delete" name="delete" type="submit" value="Delete All Galleries" />
				<input type="hidden" id="page" name="page" value="sv_manage_gallery" />
				<input type="hidden" id="sv-action" name="sv-action" value="delete-all-galleries" />
				<input type="hidden" id="referer" name="referer" value="footer" />
<?php
				wp_nonce_field('sv-delete-footer', 'sv-delete-footer-nonce');
?>
			</form>
			<input type="button" class="button" id="defaults-footer" name="defaults-footer" value="Set Defaults" onclick="location.href='<?php echo get_bloginfo('wpurl') . '/wp-admin/admin.php?page=sv_manage_gallery&sv-action=set-defaults'; ?>'" style="display: inline;" title="Set default values for gallery options." />
			<form class="sv-reset" id="reset-footer" action="<?php echo get_bloginfo('wpurl') . '/wp-admin/admin.php'; ?>" method="get" style="display: inline;" title="Reset default values of gallery options to original values." />
				<input class="button" id="reset" name="reset" type="submit" value="Reset Defaults" />
				<input type="hidden" id="page" name="page" value="sv_manage_gallery" />
				<input type="hidden" id="sv-action" name="sv-action" value="reset-defaults" />
				<input type="hidden" id="referer" name="referer" value="footer" />
<?php
				wp_nonce_field('sv-reset-footer', 'sv-reset-footer-nonce');
?>
			</form>
		</div>
<?php
	...
	function display_gallery_edit_form($gallery_id) {
		...
		$gallery_filename = $this->get_gallery_path() . $gallery_id . '.xml';
		if (file_exists($gallery_filename)) {
			$gallery_xml = simplexml_load_file($gallery_filename);
			$custom_values = $gallery_xml->attributes();
		}
		$pro_options = $this->get_pro_options($custom_values);
?>
		<h3>Edit Gallery Id <?php echo $gallery_id; ?></h3>
...
<?php
			wp_nonce_field('sv-defaults', 'sv-defaults-nonce');
?>
		</form>
<?php
	}
	...
	function shortcode_handler($atts) {
		...
		if ($gallery_id != 0) {
			...
			if (file_exists($gallery_filename)) {
				...
				$e_g_width = $settings_tag->getAttribute('e_g_width');
				$gallery_width = $e_g_width == '' ? '100%' : $e_g_width;

				$e_g_height = $settings_tag->getAttribute('e_g_height');
				$gallery_height = $e_g_height == '' ? '600px' : $e_g_height;

				$background_color = $settings_tag->getAttribute('e_bgColor');

				$e_use_flash = $settings_tag->getAttribute('e_useFlash');
				$use_flash = $e_use_flash == '' ? 'true' : $e_use_flash;
...
EOF;
				...
				return '<div><p>WP-SimpleViewer Gallery Id ' . $gallery_id . ' has been deleted.</p></div>';
			}
		} else {
			return '<div><p>WP-SimpleViewer Gallery Id cannot be found.</p></div>';
		}
	}
}
...
function sv_check_dependency() {
	...
	$upload_dir = wp_upload_dir();
	$simpleviewer_dir = $upload_dir['basedir'] . '/';

	clearstatcache();
	
	// Create uploads folder and assign full access permissions
	if (!file_exists($simpleviewer_dir))
	{
		$old = umask(0);
		if (!@mkdir($simpleviewer_dir, 0777, true)) {
		...
		if (substr(sprintf('%o', fileperms($simpleviewer_dir)), -4) != '0777') {
			$old = umask(0);
			if (!@chmod($simpleviewer_dir, 0777)) {
